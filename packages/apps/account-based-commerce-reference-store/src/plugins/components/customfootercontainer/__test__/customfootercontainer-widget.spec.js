...
import {act, cleanup, render, screen, waitFor} from '@oracle-cx-commerce/test/component/custom-render';
...
export const customfootercontainer = () => import('../index');

describe('customfootercontainer renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {customfootercontainer};
  beforeAll(async () => {
    /**
  ...
  it('customfootercontainer works as expected', async () => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'customfootercontainer',
            regions: []
          }
        }
      }
